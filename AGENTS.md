...
- **Time inputs** accept epoch seconds, RFC3339, Go duration strings relative to now (`5m`, `1h30m`), *or* Grafana's relative idiom (`now`, `now-1h`, `now+1h`; the `now` token is case-insensitive, the duration units are not). Use `ParseTimestampOrDuration` (`pkg/prometheus`) / `parseTimeWithDefault` (`handlers.go`) — don't hand-parse.
- **HTTP plumbing.** `s.GetAPIClient(ctx)` returns both the prom Go client *and* the matching `http.RoundTripper`, request-scoped when an `Authorization` header is present, so going around it bypasses per-request auth. The header is resolved per request by `authForwardingMiddleware` (MCP receiving middleware) from the SDK's `req.GetExtra().Header`; requests that carry no header use the default client built from flags. For anything outside `promv1.API`, call `s.doHTTPRequest(ctx, method, rt, path, expectJSON)`, or `s.doManagementAPICall(ctx, method, path)` for `/-/...` endpoints — both share the connection pool, emit `target_path`-labelled telemetry, and surface 404s as `ErrEndpointNotSupported`. Reference implementations: `ThanosStoresHandler` (`/api/v1/...`), the management API handlers (`/-/...`).
